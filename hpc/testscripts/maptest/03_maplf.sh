...
align_reads(){
  ...
  output=$3
    ...
    bowtie2 --local --fast-local -x $index -U $input -S $output
}
...
for sample in $samples; do
  samtools merge -o ${sample}_lf_merged.bam ${sample}_*_lf.bam
done

#mapping summary
for f in *_lf_merged.bam;do
  input=$f
  summarise_mapping $input
done > 03_summary_lf.log

cat 03_summary_lf.log | awk 'OFS="\t"{print $1,$37}' | sed 's/(//' | sed 's/%//' > mapping_rates_lf.tsv
