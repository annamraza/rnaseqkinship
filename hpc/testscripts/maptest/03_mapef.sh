...
for sample in $samples; do
  samtools merge -o ${sample}_ef_merged.bam ${sample}_*_ef.bam
done

#mapping summary
for f in *_ef_merged.bam;do
  input=$f
  summarise_mapping $input
done > 03_summary_ef.log

cat 03_summary_ef.log | awk 'OFS="\t"{print $1,$37}' | sed 's/(//' | sed 's/%//' > mapping_rates_ef.tsv
