...
align_reads(){
  ...
  output=$3
    ...
    bowtie2 --very-fast -x $index -U $input -S $output
}
...
for sample in $samples; do
  samtools merge -o ${sample}_evf_merged.bam ${sample}_*_evf.bam
done

#mapping summary
for f in *_evf_merged.bam;do
  input=$f
  summarise_mapping $input
done > 03_summary_evf.log

cat 03_summary_evf.log | awk 'OFS="\t"{print $1,$37}' | sed 's/(//' | sed 's/%//' > mapping_rates_evf.tsv
