...
for sample in $samples; do
  samtools merge -o ${sample}_lsd_merged.bam ${sample}_*_lsd.bam
done

#mapping summary
for f in *_lsd_merged.bam;do
  input=$f
  summarise_mapping $input
done > 03_summary_lsd.log

cat 03_summary_lsd.log | awk 'OFS="\t"{print $1,$37}' | sed 's/(//' | sed 's/%//' > mapping_rates_lsd.tsv
